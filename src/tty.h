...
static inline bool cpr_ttyreset(tty_ctx *tty) {
...
static inline bool cpr_inittty(tty_ctx *tty, const char *path) {
    if (!tty) return false;
...
static inline bool cpr_donetty(tty_ctx *tty) {
    if (!tty) return false;
...
static inline int cpr_ttycanon(tty_ctx *tty, const char *nl) {
    char nl1 = 0, nl2 = 0;
...
static inline int cpr_ttypacket(tty_ctx *tty, size_t size, uint8_t timer) {
    if (!tty || !tty->fd)
        return -EBADF;
